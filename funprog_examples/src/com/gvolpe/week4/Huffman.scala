...
object Huffman {
  ...
  def main(args: Array[String]) {
    ...
    var thetimes = times(chars)
    println(thetimes)

    var orderedLeafs = makeOrderedLeafList(thetimes)
    println(orderedLeafs)

    println(singleton(orderedLeafs))

    // Filter leafs with weight >= 2
    println(orderedLeafs.filter(leaf => leaf.weight >= 2))

    // Filter leafs with weight == 3
    println(orderedLeafs.filter(leaf => leaf.weight == 1))

    println("Generating Code Tree...")
    println(createCodeTree(string2Chars("cursodescalaenlaplataformacoursera")))
  ...
  def courseExample = {
    ...
    println("ENCODING... 'BED'")
    println(encode(tree)("BED".toList))
  }
   ...
   */
  def until(isSingleton: List[CodeTree] => Boolean, combiner: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): CodeTree = singleton(trees) match {
    case true => trees(0)
    case false => until(isSingleton, combiner)(combiner(trees))
  }
